...
func newMockServer() *webhookServer {
	...
	return &webhookServer{
		...
		newKafkaFromCluster: kafkautil.NewMockFromCluster,
	}
}

func TestNewServer(t *testing.T) {
	server := newWebHookServer(fake.NewFakeClient(), scheme.Scheme)
	if reflect.ValueOf(server.newKafkaFromCluster).Pointer() != reflect.ValueOf(kafkautil.NewFromCluster).Pointer() {
		t.Error("Expected newKafkaFromCluster ptr -> kafkautil.NewFromCluster")
	}
}
